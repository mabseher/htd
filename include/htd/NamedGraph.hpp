    ...
    class NamedGraph
    {
        public:
            ...
            htd::id_t associatedEdgeId(const EdgeNameType & edgeName) const
            {
                if (!names_.isEdgeName(edgeName))
                {
                    throw std::logic_error("htd::id_t htd::NamedGraph<VertexNameType, EdgeNameType>::associatedEdgeId(const EdgeNameType &) const");
                }
            ...
            void removeEdge(const VertexNameType & vertexName1, const VertexNameType & vertexName2)
            {
                if (!isVertexName(vertexName1) || !isVertexName(vertexName2))
                {
                    throw std::logic_error("void htd::NamedGraph<VertexNameType, EdgeNameType>::removeEdge(const VertexNameType &, const VertexNameType &) const");
                }
