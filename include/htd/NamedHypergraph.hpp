...
namespace htd
{
    ...
    class NamedHypergraph
    {
        public:
             ...
             */
            NamedHypergraph(const htd::LibraryInstance * const manager) : base_(manager->labeledHypergraphFactory().createInstance()), names_(), vertexCreationFunction_(std::bind(&htd::IMutableLabeledHypergraph::addVertex, base_))
            {
            ...
            void removeEdge(const std::vector<VertexNameType> & elements)
            {
                ...
                {
                    HTD_ASSERT(isVertexName(vertex))
