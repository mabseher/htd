...
std::size_t htd::TreeDecomposition::joinVertexCount(htd::vertex_t vertex) const
{
    HTD_ASSERT(isVertex(vertex))
...
htd::ConstCollection<htd::vertex_t> htd::TreeDecomposition::joinVertices(htd::vertex_t vertex) const
{
    HTD_ASSERT(isVertex(vertex))

    htd::VectorAdapter<htd::vertex_t> ret;

    copyJoinVerticesTo(ret.container(), vertex);

    return htd::ConstCollection<htd::vertex_t>::getInstance(ret);
}

void htd::TreeDecomposition::copyJoinVerticesTo(std::vector<htd::vertex_t> & target, htd::vertex_t vertex) const
{
    HTD_ASSERT(isVertex(vertex))

    if (childCount(vertex) >= 2)
    {
        std::unordered_set<htd::vertex_t> result;

        std::vector<htd::vertex_t> children;

        copyChildrenTo(children, vertex);
        ...
        std::vector<htd::vertex_t> sortedResult(result.begin(), result.end());
...
htd::vertex_t htd::TreeDecomposition::joinVertexAtPosition(htd::vertex_t vertex, htd::index_t index) const
{
    const htd::ConstCollection<htd::vertex_t> & joinVertexCollection = joinVertices(vertex);
...
bool htd::TreeDecomposition::isJoinVertex(htd::vertex_t vertex, htd::vertex_t joinVertex) const
{
    const htd::ConstCollection<htd::vertex_t> & joinVertexCollection = joinVertices(vertex);

    return std::binary_search(joinVertexCollection.begin(), joinVertexCollection.end(), joinVertex);
}
