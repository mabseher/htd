...
void htd_main::LpFormatGraphToTreeDecompositionProcessor::registerParsingCallback(const std::function<void(std::size_t, std::size_t)> & callback)
{
    implementation_->parsingCallbacks_.push_back(callback);
}

void htd_main::LpFormatGraphToTreeDecompositionProcessor::registerPreprocessingCallback(const std::function<void(std::size_t, std::size_t)> & callback)
{
