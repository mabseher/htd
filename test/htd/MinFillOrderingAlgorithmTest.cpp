...
std::size_t fillValue(const htd::IMultiHypergraph & graph, htd::vertex_t vertex)
{
    std::size_t ret = 0;

    for (htd::vertex_t neighbor1 : graph.neighbors(vertex))
    {
        for (htd::vertex_t neighbor2 : graph.neighbors(vertex))
        {
...
{
    std::size_t minFillValue = (std::size_t)-1;

    std::vector<htd::vertex_t> ret;

    for (htd::vertex_t vertex : graph.vertices())
    {
        std::size_t currentFillValue = fillValue(graph, vertex);

        if (currentFillValue <= minFillValue)
        {
            if (currentFillValue < minFillValue)
            {
                ret.clear();

                minFillValue = currentFillValue;
            }
...
TEST(MinFillOrderingAlgorithmTest, CheckCloneMethod)
{
    ...
    htd::MinFillOrderingAlgorithm algorithm(libraryInstance1);

    htd::MinFillOrderingAlgorithm * clonedAlgorithm = algorithm.clone();
...
TEST(MinFillOrderingAlgorithmTest, CheckSize50Graph1)
{
    htd::LibraryInstance * libraryInstance = htd::createManagementInstance(htd::Id::FIRST);
...
TEST(MinFillOrderingAlgorithmTest, CheckSize50Graph2)
{
    htd::LibraryInstance * libraryInstance = htd::createManagementInstance(htd::Id::FIRST);
